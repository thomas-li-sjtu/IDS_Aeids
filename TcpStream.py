...
def __calculate_byte_frequency__(payload, length):  # 计算字符频率
...
class TcpStream(threading.Thread):
                    ...
                    if to_server:
                        if self.server_last_seq < segment.get_th_seq():
                            ...
                            self.server_buffer.append(
                                (segment.get_th_seq(), segment.get_th_ack(), segment.get_data_as_string()))
                            ...
                            for i in range(0, len(self.server_buffer)):  # check for retransmission
                                ...
                                if segment_tuple[0] == segment.get_th_seq() and segment_tuple[
                                    1] == segment.get_th_ack() and len(segment_tuple[2]) == len(
                                        segment.get_data_as_string()):  # a retransmitted packet
                                    # print("retransmitted")
    ...
    def get_hexlify_payload(self, dest):  # 返回的二进制数据的十六进制
