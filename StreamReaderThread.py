...
class StreamReaderThread(threading.Thread):
    def __init__(self, filename, protocol, port):
        ...
        self.condition_lock = threading.Condition()  # Condition对象提供了对复杂线程同步问题的支持
        self.delete_read_connections = False
        self.last_timestamp = -1
        self.packet_counter = 0

        print("{} dst port {}".format(self.protocol, self.port))
        self.pcap.setfilter("{} port {}".format(self.protocol, self.port))  # 过滤
    ...
    def parse_packet(self, header, frame):  # pcap语法分析
        ...
        ts = float(str(header.getts()[0]) + "." + str(header.getts()[1]))  # packet的时间戳
    ...
    def move_stream(self, id):  # 统计完了一个流
    ...
    def has_ready_message(self):
        ...
        if not self.delete_read_connections:
            if len(self.ready_tcp_buffer)-1 == self.last_read_index:  # 读到buffer中的最后一个元素
                # self.release_lock()
                return False
            else:
                # self.release_lock()
